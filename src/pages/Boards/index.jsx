...
function Boards() {
  ...
  useEffect(() => {
    ...
    // setTotalBoards(100)
    // // Mỗi khi cái url thay đổi vd: như cta chuyển trang thì cái location.search lấy từ hook
    // useLocation của react-route-dom cũng thay đổi theo, dồng nghĩa hàm useEffect sẽ chạy lại và
    //  fetch lại api thoe đúng page mới vì cái location đã nằm trong dependenci của useEffect

    // Gọi API lấy danh sách boards ở đây...
    fetchBoardsAPI(location.search).then(updateStateData)
  }, [location.search])

  const afferCreateNewBoard = () => {
    // Đơn giản là cứ fetch danh sách board tương tự useEffect
    fetchBoardsAPI(location.search).then(updateStateData)
  }
      ...
      <Box sx={{ paddingX: 2, my: 4 }}>
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, sm: 3 }}>
            ...
            <Divider sx={{ my: 1 }} />
            <Stack direction="column" spacing={1}>
              <SidebarCreateBoardModal afferCreateNewBoard = {afferCreateNewBoard} />
            </Stack>
          </Grid>
