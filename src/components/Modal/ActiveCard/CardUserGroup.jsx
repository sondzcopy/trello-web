...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMember }) {
  ...
  // Đoạn này lấy activeBoard từ redux ra để lấy đc toàn bộ thành viên của cái board thông qua FE_ALL_USER
  const board = useSelector(selectCurrentActiveBoard)
  /**
   * Thành viên trong card sẽ phải là tập con của tv tỏng Board
   * Vì thế dựa vào mảng board.FE_allUsers và card.memberIds rồi ct tạo ra 1 mảng FE_CardMember chứa đủ thông tin của user để hiển thị ra ngoài giao diện, rồi mặc định trong card chỉ lưu Id của user thôi (memberIds)
   */
  ...
  // console.log('FE_CardMember,', FE_CardMembers)

  const handleUpdateCardMember = (user) => {
    // console.log('first', user)
    // Tạo một biến incomingMemberInfo để gửi cho BE 2 thông tin chính là user và action xóa khỏi card hoặc thêm vào card
